...
public class RealmSwiftAdapter: NSObject, ModelAdapter {
    ...
    func getRecord(for syncedEntity: SyncedEntity) -> CKRecord? {
        ...
        if let recordData = syncedEntity.record?.encodedRecord {
            if let unarchiver = try? NSKeyedUnarchiver(forReadingFrom: recordData){
...
//    }
    ...
    public var serverChangeToken: CKServerChangeToken? {
        ...
        executeOnMainQueue {
            ...
            if let tokenData = serverToken?.token {
                token = try? NSKeyedUnarchiver.unarchivedObject(ofClass: CKServerChangeToken.self, from: tokenData)
            }
        }
        return token
    }
    ...
    public func saveToken(_ token: CKServerChangeToken?) {
        ...
        executeOnMainQueue {
            ...
            if let token = token {
                serverToken.token = try? NSKeyedArchiver.archivedData(withRootObject: token, requiringSecureCoding: false)
            } else {
